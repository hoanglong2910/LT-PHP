            ...
            <tbody class="divide-y divide-gray-100">
               <tr v-for="bh in baohiem" :key="bh.id" class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-3 text-right">
                     <inertia-link :href="route('baohiem.edit', bh.id)" class="text-gray-400 hover:text-indigo-600">
          ...
          <table class="w-full whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-100">
               <tr v-for="hd in hopdong" :key="hd.id" class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-3 text-gray-600">{{ hd.ngaybd }}</td>
                  <td class="px-6 py-3 text-gray-600">{{ hd.ngaykt }}</td>
                  <td class="px-6 py-3 text-right">
                     <inertia-link :href="route('hopdong.edit', hd.id)" class="text-gray-400 hover:text-indigo-600">
                        <icon name="cheveron-right" class="w-5 h-5" />
                     </inertia-link>
                  </td>
               </tr>
               <tr v-if="hopdong.length === 0">
                  <td colspan="5" class="px-6 py-6 text-center text-gray-400 italic">Chưa có hợp đồng nào.</td>
               </tr>
            </tbody>
          </table>
        </div>
...
import TextInput from '@/Shared/StyledTextInput' // Đã dùng StyledTextInput của bạn
...
export default {
  ...
  data() {
    return {
      ...
      form: this.$inertia.form({
        _method: 'put',
        manv: this.nhanvien.manv, // Cần thêm trường này vào Controller nếu chưa có
  ...
  methods: {
    update() {
      ...
      // Lưu ý: post thay vì put để gửi được file
