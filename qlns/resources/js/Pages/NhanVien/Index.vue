                ...
                <div class="flex items-center">
                   <div class="flex-shrink-0 h-10 w-10">
                      ...
                      <div v-else class="h-10 w-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-md">
                         {{ nv.hovaten ? nv.hovaten.charAt(0).toUpperCase() : 'NV' }}
                      </div>
                   </div>
                   <div class="ml-4">
        ...
        <div class="px-6 py-6">
           <form @submit.prevent="storeImport">
              <div class="mb-4">
                 ...
                 <file-input v-model="form.fileimport" class="w-full" type="file" accept=".xlsx, .xls" />
              ...
              <div class="flex justify-end gap-3 mt-6">
                 ...
                 <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium text-sm shadow-md transition-colors flex items-center" :disabled="isImporting">
                    <span v-if="isImporting" class="mr-2">
                        <svg class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    </span>
                    <span>{{ isImporting ? 'Đang tải...' : 'Tải lên' }}</span>
                 </button>
              </div>
           </form>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      showImportModal: false, // Quản lý hiển thị modal
      isImporting: false,     // Trạng thái loading khi import
    }
  },
  watch: {
    form: {
      handler: throttle(function() {
        // Tách fileimport ra khỏi query string để URL sạch đẹp
  ...
  methods: {
    storeImport() {
      ...
      axios.post('/nhanvien/import', formData, {
        ...
        this.closeModal();
        window.location.reload(); // Reload để thấy dữ liệu mới
      })
      .catch((error) => {
        console.error(error);
    ...
    closeModal() {
      ...
      this.form.fileimport = null; // Reset file khi đóng modal
    }
  },
}
</script>