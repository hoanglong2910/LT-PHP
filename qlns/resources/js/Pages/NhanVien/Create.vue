  ...
  <div>
    <div class="mb-8 flex justify-between items-center">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Tạo hồ sơ nhân viên mới và tài khoản hệ thống.</p>
      </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <text-input v-model="form.email" :error="form.errors.email" label="Email đăng nhập" type="email" placeholder="email@example.com" />
                 <text-input v-model="form.password" :error="form.errors.password" type="password" label="Mật khẩu" autocomplete="new-password" placeholder="Nhập mật khẩu..." />
...
import TextInput from '@/Shared/StyledTextInput' // Ưu tiên StyledTextInput từ Create.vue
...
export default {
  ...
  methods: {
    store() {
      // Logic xử lý file photo trước khi gửi
      if (this.$refs.photo && this.$refs.photo.files[0]) {
        this.form.photo = this.$refs.photo.files[0]
      }
      this.form.post(this.route('nhanvien.store')) // Dùng route name an toàn hơn
    },
    selectNewPhoto() {
