        ...
        >
          <div class="flex items-center">
            <icon
              v-if="item.iconName"
              :name="item.iconName"
              class="w-5 h-5 mr-4 transition-colors duration-200 fill-current"
          ...
          <icon
            ...
            name="cheveron-down"
            class="w-3 h-3 transition-transform duration-300 fill-current"
...
export default {
  ...
  data() {
    return {
      ...
      // GIỮ NGUYÊN DANH SÁCH MENU TỪ MAINMENU1
      menuItems: [
        ...
        { label: 'QUẢN LÝ THÔNG TIN', isHeading: true, requiredRoleGT: 0 },
        {
          ...
          children: [
            ...
            { label: 'Nghỉ Việc', routeName: 'nghiviec', requiredRoleGT: 0, checkActive: (pageUrl) => pageUrl.startsWith('nghiviec') },
          ],
        },
        {
          label: 'Quản lý KPI',
        ...
        },
        {
          ...
          children: [
            { label: 'Danh sách Hợp Đồng', routeName: 'hopdong', requiredRoleGT: 0, checkActive: (pageUrl) => pageUrl.startsWith('hopdong') },
            { label: 'Bảo Hiểm', routeName: 'baohiem', requiredRoleGT: 0, checkActive: (pageUrl) => pageUrl.startsWith('baohiem') },
            { label: 'Khẩu Trừ', routeName: 'khautru', requiredRoleGT: 0, checkActive: (pageUrl) => pageUrl.startsWith('khautru') },
          ],
        },
        { label: 'CHÍNH SÁCH & TÀI CHÍNH', isHeading: true, requiredRoleGT: 0 },
        {
          ...
          children: [
            { label: 'Danh sách Hệ Số', routeName: 'heso', requiredRole: 2, checkActive: (pageUrl) => pageUrl.startsWith('heso') },
            { label: 'Phụ Cấp', routeName: 'phucap', requiredRole: 2, checkActive: (pageUrl) => pageUrl.startsWith('phucap') },
          ],
        },
        {
          ...
          children: [
            { label: 'Xem Bảng Chấm Công', routeName: 'bangchamcong', requiredRole: 2, checkActive: (pageUrl) => pageUrl.startsWith('bangchamcong') },
        ...
        },
        {
          ...
          children: [
            { label: 'Nhận Lương', routeName: 'nhanluong', requiredRoleGT: 0, checkActive: (pageUrl) => pageUrl.startsWith('nhanluong') },
            { label: 'Ứng Lương', routeName: 'ungluong', requiredRoleGT: 0, checkActive: (pageUrl) => pageUrl.startsWith('ungluong') },
          ],
        },
      ],
    }
  ...
  methods: {
    // LOGIC PHÂN QUYỀN TỪ MAINMENU1 (Rất quan trọng)
    shouldShowItem(item) {
  ...
  mounted() {
    // Tự động mở menu cha nếu có menu con đang active khi load trang
