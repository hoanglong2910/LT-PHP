...
use App\Models\AiEvaluation; // Model đánh giá AI
...
class NhanVienController extends Controller
{
    ...
    public function create()
    {
        return Inertia::render('NhanVien/Create', [
            ...
            'bangcap' => (new BangCap())->orderBy('tenbc')->get()->map->only('id', 'tenbc'),
            'chuyenmon' => (new ChuyenMon())->orderBy('tencm')->get()->map->only('id', 'tencm'),
            'ngoaingu' => (new NgoaiNgu())->orderBy('tenng')->get()->map->only('id', 'tenng'),
            'dantoc' => (new DanToc())->orderBy('tendt')->get()->map->only('id', 'tendt'),
            'tongiao' => (new TonGiao())->orderBy('tentg')->get()->map->only('id', 'tentg')
        ]);
    }
    ...
    public function store()
    {
        ...
        // Tạo User và NhanVien
        // Lưu ý: Logic này giả định Auth::user()->nhanvien tồn tại và có quan hệ đúng
        Auth::user()->nhanvien->user->create([
            ...
            'password' => Request::get('password'), // Nên hash password ở Model User hoặc tại đây
    ...
    public function update(NhanVien $nhanvien)
    {
        ...
        if (Request::file('photo')) {
            $nhanvien->update(['photo_path' => Request::file('photo')->store('nhanvien')]);
        }
